...
public abstract class BaseFakeCardTest {
    ...
    BaseFakeCardTest(FakeCard card) {
    ...
    void returns_firmware_version() {
        ResponseAPDU response = testUtil.sendToCard("FF 00 48 00 00");
    ...
    void returns_current_picc() {
        ResponseAPDU response = testUtil.sendToCard("FF 00 50 00 00");
    ...
    void can_change_picc() {
        // Change PICC to 0xCC
        ResponseAPDU response = testUtil.sendToCard("FF 00 51 CC 00");
        assertThatResponseBytes(response)
                .isEqualTo("90 CC");

        // Read PICC after change
        response = testUtil.sendToCard("FF 00 50 00 00");
        assertThatResponseBytes(response)
                .isEqualTo("90 CC");
    }
    ...
    void can_set_timeout() {
        ...
        ResponseAPDU response = testUtil.sendToCard("FF 00 41 08 00");
        assertThatResponseBytes(response)
                .isEqualTo(RESP_OK);
    }
